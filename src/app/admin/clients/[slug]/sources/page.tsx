  ...
  return (
    <div>
      ...
      <section className="mt-8 rounded-xl border border-dashed border-neutral-300 bg-white p-5">
        <h2 className="text-base font-semibold">Bulk add URLs</h2>
        <p className="mt-1 text-xs text-neutral-500">
          Paste one URL per line (or comma-separated). Each URL is added as a single-page source and queued for indexing immediately.
        </p>
        <form
          action={async (fd) => {
            "use server";
            await bulkAddSourcesAction(slug, fd);
          }}
        ...
        >
          <textarea
            name="urls"
            rows={5}
            placeholder={"https://example.com/referral-form\nhttps://example.com/patient-portal\nhttps://example.com/services"}
            required
            className="w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm font-mono outline-none focus:border-neutral-500"
          />
      ...
      <section className="mt-4 rounded-xl border border-dashed border-neutral-300 bg-white p-5">
